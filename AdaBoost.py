...
#Adaboost is not proon to overfitting
...
def run_classifier():
    ...
    # copper_wire_df = pd.read_csv("data_from_sas.csv")
    ...
    # case with the use of logistic regressiong
    # logregmodel = LogisticRegression() #gives worse results
    # ada_classifier = AdaBoostClassifier(n_estimators=50, base_estimator=logregmodel, learning_rate=1) #seems to be const when changing n_estimators


    # Train Adaboost Classifier
    model = ada_classifier.fit(X_train, y_train)

    # Predict the response for test dataset
    y_predict_ada = model.predict(X_test)
    print("Adaboost Classifier")
    print("Accuracy is: ", metrics.accuracy_score(y_test, y_predict_ada))
...
#recall how many of the observed cases were correctly predicted as positive (third bottom value)
# f meaure is aggregation of the both to one metric (harmonic mean)
#support is
